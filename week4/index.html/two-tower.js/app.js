...
class MovieLensApp {
    constructor() {
        // Data and Indexing
        this.interactions = []; // Original interactions
        this.indexedInteractions = []; // Interactions with 0-based indices
        this.items = new Map(); // Map<OriginalItemID, {title, year, ...}>
        this.userIds = []; // Array of unique user IDs
        this.itemIds = []; // Array of unique item IDs
        this.userIdxMap = new Map(); // Map<OriginalUserID, 0-based index>
        this.itemIdxMap = new Map(); // Map<OriginalItemID, 0-based index>
        this.userTopRated = new Map(); // Map<OriginalUserID, ratings array> for test comparison
        this.model = null;
        
        // Configuration
        this.config = {
            maxInteractions: 80000, // Use a subset of 100K for faster training
            embeddingDim: 32,
            batchSize: 512,
            epochs: 10,
            learningRate: 0.001
    ...
    async loadData() {
        ...
        try {
            ...
            this.updateStatus(`Error loading data. Check console. Error: ${error.message}`);
            this.setControls(false, false);
        }
    }
    ...
    async train() {
        ...
        // Final step: Visualize embeddings
    ...
    plotLoss(history) {
        ...
        // Drawing logic (omitted for brevity, but implemented in original plan)
        // ... (standard canvas drawing for line chart)
    ...
    async visualizeEmbeddings() {
        ...
        projection.forEach(p => {
            ...
            const y = padding + plotHeight * (1 - (p[1] - minY) / yRange); // Flip Y-axis
        ...
        canvas.onmousemove = (event) => {
            ...
            // Simple nearest neighbor search on sampled points for hover info
    ...
    async test() {
        ...
        userEmbedding.dispose(); // Clean up tensor
...
// Initialize app when page loads and set WebGL backend for performance
let app;
